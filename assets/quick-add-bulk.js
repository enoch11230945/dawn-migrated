    ...
    class QuickAddBulk extends BulkAdd {
      ...
      listenForActiveInput() {
        if (!this.classList.contains('hidden')) {
          // FIXED v18.0: Create bound handler once to prevent listener stacking.
          if (!this._focusinHandler) {
            this._focusinHandler = (event) => event.target.select();
          }
          // Remove existing before adding to prevent duplicates
          this.input?.removeEventListener('focusin', this._focusinHandler);
          this.input?.addEventListener('focusin', this._focusinHandler);
        }
