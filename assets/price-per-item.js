      ...
      updatePricePerItem(updatedCartQuantity) {
        ...
        for (let pair of this.qtyPricePairs) {
          if (this.currentQtyForVolumePricing >= pair[0]) {
            const pricePerItemCurrent = document.querySelector(`price-per-item[id^="Price-Per-Item-${CSS.escape(this.dataset.sectionId || this.dataset.variantId)}"] .price-per-item span`);
            // FIXED: Use textContent instead of innerHTML to prevent XSS from price data.
            // Original: pricePerItemCurrent.innerHTML = pair[1]
            if (pricePerItemCurrent) {
              pricePerItemCurrent.textContent = this.classList.contains('variant-item__price-per-item')
